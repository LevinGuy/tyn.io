...
var sessionStore = sessions.NewCookieStore(
	// New Keys
	[]byte(os.Getenv("TYNIO_SESSION_AUTH_KEY_V2")),
	[]byte(os.Getenv("TYNIO_SESSION_ENC_KEY_V2")),
	// Old Keys
	[]byte(os.Getenv("TYNIO_SESSION_AUTH_KEY_V1")),
	[]byte(os.Getenv("TYNIO_SESSION_ENC_KEY_V1")),
)
...
func main() {
	// setup
	baseUrl := os.Getenv("TYNIO_BASE_URL")
	port := os.Getenv("TYNIO_PORT")
	if port == "" {
		log.Fatal("Specify a port to listen on in the environment variable 'TYNIO_PORT'")
	}
	dbDumpDir := os.Getenv("TYNIO_DB_DUMP_DIR")
	...
	boltStore := store.NewBoltStore("tynio.db")
	...
	// Twitter
	twitterConsumerKey := os.Getenv("TYNIO_TWITTER_CONSUMER_KEY")
	twitterConsumerSecret := os.Getenv("TYNIO_TWITTER_CONSUMER_SECRET")
	...
	// Google (Plus)
	//
	gplusClientId := os.Getenv("TYNIO_GPLUS_CLIENT_ID")
	gplusClientSecret := os.Getenv("TYNIO_GPLUS_CLIENT_SECRET")
	if gplusClientId != "" {
	...
	// GitHub
	githubClientId := os.Getenv("TYNIO_GITHUB_CLIENT_ID")
	githubClientSecret := os.Getenv("TYNIO_GITHUB_CLIENT_SECRET")
	if githubClientId != "" {
